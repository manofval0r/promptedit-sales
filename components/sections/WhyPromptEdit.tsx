    ...
    <section className="py-24 px-5 md:px-8 lg:px-16 max-w-[1200px] mx-auto relative">
      <div className="flex flex-col items-center text-center gap-4 mb-16">
        ...
        <ScrollReveal delay={100}>
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-text-primary max-w-2xl">
            {WHY.heading}
          </h2>
        </ScrollReveal>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        {largeFeature && (
          <div className="lg:col-span-5 flex">
            <ScrollReveal delay={200} className="w-full">
              <GlowCard className="w-full h-full flex flex-col justify-center gap-8 bg-bg-surface/80 border-accent-primary/30 shadow-[0_0_30px_var(--accent-glow)] hover:border-accent-primary/60">
        ...
        {/* Right Column: Stacked Smaller Features */}
        <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {smallFeatures.map((feature, idx) => (
            <ScrollReveal key={idx} delay={300 + idx * 100} className="h-full">
              <GlowCard className="h-full flex flex-col gap-4 p-6 bg-bg-surface/60 backdrop-blur-sm">
